...
-(id) initWithFrame:(CGRect)frame addTarget:(BrowserViewController *) vc {
	if ((self = [super initWithFrame:frame])) {
        ...
        if (vc.tabs.count>=1) {
            //Not add close button on first tab
            [self addSubview:closeButton];
        }
        tabButton.tag=100+vc.tabs.count;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if ([pageData length] == 0) {
        ...
        if (current && ![currentURLString isEqualToString:@"about:blank"] && [currentURLString rangeOfString:@" "].location == NSNotFound) {
...
- (BOOL)isWebViewFirstResponder
{
    ...
    js = @"document.getElementById(\"main\") ? document.getElementById(\"main\").offsetHeight : -1";
...
-(BOOL) webView:(UIWebView*)webView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType {
   ...
   // CAPTURE PAGE LOAD
    ...
    if ([presentURL rangeOfString:@".pdf" options:NSCaseInsensitiveSearch].location != NSNotFound)
    {
        NSString *urlStr = [NSString stringWithFormat:@"%@",appdelegate.prevAddress];
        
        if([urlStr isEqualToString:presentURL])
        {
            return YES;
        }else
        {
            appdelegate.prevAddress = presentURL;
            urlPresent = [NSString stringWithFormat:@"%@",presentURL];
            [viewController addTabWithAddress:presentURL];
...
-(void) webViewDidFinishFinalLoad:(UIWebView *)webView {
    ...
    NSArray *abc = [url componentsSeparatedByString:@"//"];
    if ([tabTitle length] == 0) {
        [self setTitle:[NSString stringWithFormat:@"%@",[abc objectAtIndex:1]]];
    } else {
        [self setTitle:[NSString stringWithFormat:@"%@",[abc objectAtIndex:1]]];
    }
